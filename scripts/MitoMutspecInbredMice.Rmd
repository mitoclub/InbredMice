...
```{r}
hist(mother$AltVAF, breaks = 20)
```
```{r}
MtDna$MutID <- paste(MtDna$Pos, MtDna$SNP, sep = '_')
length(unique(MtDna$MutID))
MtDnaShort = MtDna[!(MtDna$MutID %in% MtDna[MtDna$NameNew == 'C57BL',]$MutID),] # 21 > 1737-1420
MtDnaShort2 = MtDna[!(MtDna$MutID %in% MtDna[(MtDna$NameNew == 'C57BL') & (MtDna$AltVAF > 0.05),]$MutID),] # 21 > 1737-1596

barplot(table(MtDnaShort[MtDnaShort$AltVAF >= 0.03,]$SNP))
```
...
```{r}
mutID_Freq <- data.frame(table(MtDnaShort$MutID))
hist(mutID_Freq$Freq, breaks = 50)
```
